...
def test_troops_editor_script_is_served(tmp_path: Path) -> None:
    c = _client(tmp_path)
    assert 'src="/static/troops.js"' in c.get("/inventory/troops").text
    r = c.get("/static/troops.js")
    assert r.status_code == 200
    assert "/api/troops" in r.text
...
def test_shared_app_js_is_loaded_by_every_shell_page(tmp_path: Path) -> None:
    ...
    # class, and textContent set only once it is visible.
    unhide = r.text.index("hidden = false")
    assert r.text.index("textContent", unhide) > unhide
...
def test_hero_levels_page_has_spend_form(tmp_path: Path) -> None:
    body = _client(tmp_path).get("/optimiser/hero-levels").text
    assert "Hero EXP available" in body
    assert "Find best spends" in body
    assert "/api/optimize/hero-levels" in body


def test_hero_levels_api_returns_spend_pack(tmp_path: Path) -> None:
    heroes_dir = tmp_path / "heroes"
    heroes_dir.mkdir()
    store = HeroStore(heroes_dir)
    for name in _CATALOG_HEROES:
        store.upsert(
            HeroRecord(name=name, stars=3, power=300_000, level=10, scraped_at="t")
        )
    c = TestClient(create_app(heroes_dir=heroes_dir))
    r = c.post(
        "/api/optimize/hero-levels",
        json={"event": "swordland", "hero_exp": 0},
    )
    assert r.status_code == 200, r.text
    data = r.json()
    assert "baseline_utility" in data
    assert "best_utility" in data
    assert "steps" in data
    assert data["leftover_exp"] == 0
    assert data["steps"] == []
