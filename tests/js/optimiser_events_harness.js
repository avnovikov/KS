...
function makePage(spec) {
  ...
  return {
    ...
    /** The *rendered text* of one half of a chip, dug out of the innerHTML
     *  blob the script assembles.
     *
     *  Reading the blob directly is a trap this suite fell into: a chip is
     *  `<span class="mode-name">…</span><span class="mode-score">…</span>`,
     *  so `chipText(i).indexOf("score") !== -1` is satisfied by the *class
     *  attribute* on every chip ever rendered, points or score. Deleting the
     *  whole `"score " + fmtScore(...)` branch left that check green. These
     *  two pull out the span contents so a needle has to match what the user
     *  actually reads. */
    chipPart: function (i, cls) {
      var chip = chipsEl.children[i];
      if (!chip) return "";
      var m = new RegExp('<span class="' + cls + '">([\\s\\S]*?)</span>').exec(chip.innerHTML);
      return m ? m[1] : "";
    },
    chipName: function (i) {
      return this.chipPart(i, "mode-name");
    },
    chipScore: function (i) {
      return this.chipPart(i, "mode-score");
    },
...
/**
 * The Conquest section, transcribed from CombatFormationResult.to_dict() as
 * run_optimize_bundle emits it — not from the arena fixture above, because
 * the two differ in exactly the ways that matter here:
 *
 *   - no `side` key at all (optimize_conquest passes side=None, and to_dict
 *     omits it), so the board must not reach for one;
 *   - no `explanations` key either: optimize_conquest is the one caller that
 *     sets with_explanations=False, so the hero sheet falls through to its
 *     "no explanation recorded" branch. The legacy #conquest-block page did
 *     the same. Faking explanations here would hide that;
 *   - `heroes` is a list of bare name strings, as arena's is — not the
 *     {name, explain} objects the sword/bear modes carry.
 *
 * `gear_assignment` is populated because attach_gear_icon_urls patches the
...
async function suiteArenaFormation() {
  ...
  check(
    ...
    // Was `chipText(0).indexOf("score") !== -1`, which the `class="mode-score"`
    // in the same blob satisfied on its own — see chipPart(). Pinned on the
    // rendered value now, so deleting chipScoreText's scored branch fails it.
    d.chipScore(0) === "score 636.4",
    d.chipScore(0) + " | " + d.chipText(0)
  );
  check("and no points figure on it", d.chipScore(0).indexOf("pts") === -1, d.chipScore(0));
...
async function suiteConquestFormation() {
  var d = makePage({ bundle: goodBundle() });
  await boot(d);
  d.eventButton("conquest").fire("click");

  check(
    "Conquest is a board of its own, titled from its segment",
    d.boardTitle() === "Conquest · formation",
    d.boardTitle()
  );
  check(
    "it is one formation, not a list of modes",
    d.chips().length === 1,
    "chips=" + d.chips().length
  );
  check("its single chip is selected", d.selectedChip() === d.chips()[0], d.chipText(0));
  check("named for what it is", d.chipName(0) === "formation", d.chipName(0));
  check(
    "the chip carries the ILP score",
    d.chipScore(0) === "score 512.8",
    d.chipScore(0) + " | " + d.chipText(0)
  );
  check("and no points figure", d.chipScore(0).indexOf("pts") === -1, d.chipScore(0));

  // Exact, not a substring: this pins the *absence* of the troops and
  // breakdown tail as much as the presence of the score. optimize_conquest
  // reads no troops.yaml, so a troops line here would render as em-dashes.
  check("the meta is the score alone", d.boardMeta() === "score 512.8", d.boardMeta());
  check(
    "with no troops line, because Conquest is scored from no troop counts",
    d.boardMeta().indexOf("cap") === -1 && d.boardMeta().indexOf("I ") === -1,
    d.boardMeta()
  );
  ...
  var front = rows[0].slots
    .map(function (s) {
      return s.slotTag + "=" + s.name;
    })
    .join(",");
  var back = rows[1].slots
    .map(function (s) {
      return s.slotTag + "=" + s.name;
    })
    .join(",");
  check("Front holds the two F slots", front === "F1=Amadeus,F2=Hilde", front);
  check("Back holds the three B slots", back === "B1=Marlin,B2=Gordon,B3=Saul", back);
  record("conquest_front_row", front);
  record("conquest_back_row", back);
  ...
  // The sheet, on a section whose heroes are bare name strings and which
  // carries no explanations at all — the shape optimize_conquest really
  ...
  check(
    ...
    d.modalBody.innerHTML.indexOf("Conqueror&#39;s Sallet") !== -1,
    d.modalBody.innerHTML.slice(0, 400)
  );
  check(
    "and says plainly that Conquest records no per-hero why",
    d.modalBody.innerHTML.indexOf("No explanation recorded for this hero.") !== -1,
    d.modalBody.innerHTML.slice(0, 400)
  );
  ...
  d.eventButton("sword").fire("click");
  check(
    "leaving Conquest restores the event before it",
    d.boardTitle() === "Swordland · garrison",
    d.boardTitle()
  );
  check(
    "and that board gets its troops line back",
    d.boardMeta().indexOf("cap") !== -1 && d.boardMeta().indexOf("pts") !== -1,
    d.boardMeta()
  );
}

async function suiteConquestFailure() {
  var bundle = goodBundle();
  bundle.errors = { conquest: "conquest broke" };
  bundle.conquest = {
    mode: "conquest",
    status: "Error",
    formation: {},
  ...
  check("the board gives the solver's reason", d.boardEmpty() === "conquest broke", d.boardEmpty());
  check("and draws no hero slots", d.rows().length === 0, "rows=" + d.rows().length);
  check(
    "the banner names it too",
    d.sectionErrEl.hidden === false && d.sectionErrEl.textContent === "conquest broke",
    "hidden=" + d.sectionErrEl.hidden + " text=" + d.sectionErrEl.textContent
  );
  check(
    "and the status line reports it",
    d.statusEl.textContent.indexOf("conquest: conquest broke") !== -1,
    d.statusEl.textContent
  );

  d.eventButton("arena").fire("click");
  check(
    "the other three events are unharmed",
    d.boardTitle() === "Arena · attack" && d.sectionErrEl.hidden === true,
    d.boardTitle() + " banner-hidden=" + d.sectionErrEl.hidden
  );

  // The other failure run_optimize_bundle can produce for this section: the
  // ILP came back Infeasible rather than throwing, so there is a status but
  // no `error` string to show.
  var infeasible = goodBundle();
  infeasible.errors = { conquest: "status=Infeasible" };
  infeasible.conquest = {
    mode: "conquest",
    status: "Infeasible",
    formation: {},
  ...
  var e = makePage({ bundle: infeasible });
  await boot(e);
  e.eventButton("conquest").fire("click");
  check(
    "an infeasible Conquest names its status on the board",
    e.boardMeta() === "status: Infeasible",
    e.boardMeta()
  );
  check(
    "and falls back to a message that does not call it a side",
    e.boardEmpty() === "No optimal formation for this lineup.",
    e.boardEmpty()
  );
  check(
    "with no banner, because the solver reported no error string",
    e.sectionErrEl.hidden === true,
    "hidden=" + e.sectionErrEl.hidden + " text=" + e.sectionErrEl.textContent
  );
}
...
(async function main() {
  var suites = [
    ...
    suiteArenaFormation,
    suiteConquestFormation,
    suiteConquestFailure,
    suiteHeroSheet,
    suiteEscaping,
