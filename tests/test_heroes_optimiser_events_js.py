...
def test_conquest_is_the_fourth_event_on_the_same_board(js_run: dict) -> None:
    """Ported from the legacy events page this merge deleted, where Conquest
    was its own `#conquest-block` section with a formation card.

    It takes the *arena* shape rather than the March single row, and that is
    forced by the data, not a layout preference: optimize_conquest returns
    the same `CombatFormationResult` that optimize_arena does, from the same
    `solve_combat_formation` ILP, over the same F1/F2 + B1/B2/B3 slot
    vocabulary. The March row exists only for rows that carry no `formation`
    at all. So the board's existing `hasFormation()` branch already draws it
    correctly; what the port had to add was the *enumeration* — Conquest is
    one row where arena is two sides and sword/bear are a map of modes.
    """
    _assert_ran(
        js_run,
        [
            ...
            "its single chip is selected",
            "named for what it is",
            "the chip carries the ILP score",
            "and no points figure",
    ...
    assert js_run["data"]["conquest_front_row"] == "F1=Amadeus,F2=Hilde"
    assert js_run["data"]["conquest_back_row"] == "B1=Marlin,B2=Gordon,B3=Saul"


def test_a_failed_conquest_surfaces_like_any_other_section(js_run: dict) -> None:
    """`errors["conquest"]` is set two different ways in run_optimize_bundle:
    a raised solver error, which puts a `status: "Error"` row with an `error`
    string in the bundle, and a solve that merely came back non-Optimal,
    which puts a status on the row and *no* error string. Both are covered —
    the second is the one that would silently render a blank board."""
    _assert_ran(
        js_run,
        [
